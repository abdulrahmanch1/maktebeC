...
const SecuritySettings = () => {
  const { theme } = useContext(ThemeContext);
  const { user, token, logout } = useContext(AuthContext); // Get user, token, and logout from AuthContext
  ...
  const handlePasswordResetRequest = async () => {
    try {
      ...
      if (!userEmail) {
        toast.error("لا يمكن إرسال رابط إعادة تعيين كلمة المرور. البريد الإلكتروني غير متاح.");
        return;
      }
      await axios.post(`${API_URL}/api/users/forgot-password`, { email: userEmail });
      ...
      toast.error(err.response?.data?.message || "فشل إرسال رابط إعادة تعيين كلمة المرور.");
    }
  };
